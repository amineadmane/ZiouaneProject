    ...
    mounted(){
      ...
      this.livres.map(function(element){
        if(element.etat2=="pret") {
         ...
         return[  _this.colisPret, _this.totalPret =parseFloat(_this.totalPret)+parseFloat(element.prix) ] 
        }
        else {
          _this.colisnonPret.push(element);
          return [ _this.colisnonPret,_this.totalProchain =parseFloat(_this.totalProchain)+parseFloat(element.prix)] 
        }
      })
