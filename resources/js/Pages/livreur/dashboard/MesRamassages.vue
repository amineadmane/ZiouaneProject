...
<v-app app>
          ...
          <v-card>
            ...
            <v-card-text>
              ...
              <template v-for="(item) in colisPret">
                <v-list-item :key="item.id_colis" v-if="item.wilaya!=undefined">
                  <template v-slot:default>
                    <v-list-item-content>
                      <v-list-item-title>
                        ...
                        <v-chip v-if="item.livraison==1"
                         color="primary"
                        >
                        Livraison Gratuite {{item.fraisLivraison}} DZD
                        </v-chip>
                        <br>
                        <br>
                        {{item.nomclient}}
    ...
    <layout>
      ...
      <v-main>
        ...
        <div class="ml-4 mb-2 d-flex justify-between">
    ...
    methods: {
      ...
      totalRamasser(item){
        ...
        item.forEach(element => {
          if(element.livraison==1) 
               return total = parseFloat(total)+parseFloat(element.fraisLivraison);
      ...
      genPdf(){
         if(this.selected.length==0) 
          ...
          this.selected.forEach(element => {
             ...
             Object.values(element).forEach(x => {
               if(x.wilaya != undefined) items.push(x);
             });
         ...
         if (pdfMake.vfs == undefined){
            ...
            var dd=  [
                  ...
                  this.table(items, ['ref','codecolis','nomclient', 'telephone','adresse','prix','remarque'])
                ]
      ...
      changerEtatRamassage(){
        ...
        form.selected=this.selected
         alert('dfffs');
        form.etat=this.etat
        this.$inertia.post(route('livreur.changerEtatR'),form).then(()=>{
         this.refreshRam();  
          this.selected=[]
          this.$toast.open({
      ...
      refreshRam(){
          this.$inertia.visit(route('livreur.ramassages'))
      },
      deleteItem (item) {
