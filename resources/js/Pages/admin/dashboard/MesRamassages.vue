...
<v-app app>
    ...
    <layout>
      ...
      <v-main>
        ...
        >
        <div class="ml-4">
          <v-row class=""  v-if="$page.auth.role!='super-admin'">
            <v-col cols="12" md="3">
                <h4>Assigner a un livreur</h4>
                <v-select 
                  class="mt-2"
                  :items="livreurs"
                  :item-text="text"
...
export default {
    ...
    methods: {
      ...
      table(data, columns) {
          return {
              table: {
                  widths: [75,60,60,60,95,55,60,50 ],
                  headerRows: 1,
      ...
      refreshRam(){
         this.$inertia.visit(route('admin.ramassages'))
      },
      genPdf(){
      ...
      attribuerLivreur(){
        if(this.livreur=='') 
          return  this.$toast.open({
            message: "Aucun livreur selectionné ",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        var form = {};
        var _this = this
        var liv = this.livreurs.filter(function(element){
             return element.id_livreur== _this.livreur
        });
        this.selected.forEach(element => {
          element[0].livreur  = liv
        });
        form.selected=this.selected
