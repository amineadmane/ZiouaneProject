      ...
      <v-main>
        ...
        <div class="ml-4">
          <v-row class="">
            ...
            <v-col cols="12" md="3" v-if="$page.auth.role!='agent-bureau' && $page.auth.role!='super-admin'">
                <h4 v-if="$page.auth.role=='agent-centre'">Assigner a un transporteur</h4>
                <h4 v-else>Assigner a un livreur</h4>
                <v-select 
                  class="mt-2"
                  :items="livreurs"
                  :item-text="text"
        ...
        <div class="text-right ml-4 mb-2 d-flex justify-between">
          <v-row class="">
            <v-btn
              rounded
              color="primary"
...
export default {
    ...
    mounted(){
    var _this = this;
     _this.tableItems=this.colis;
     var map = {nom:'aucun',prenom:'livreur',id_livreur:null}
     if(this.livreurs!=null) this.livreurs.push(map);
     
     if(_this.centres!=null)
     if(_this.$page.auth.role=="agent-centre") { 
       _this.items.push('au centre');
       _this.items.push('en transport');
       _this.centres.forEach(function(element){
       return _this.items.push(element['nom'])
     }) 
     this.wilayas.forEach(element => {
        return _this.items.push('bureau '+element)
     });
     ...
     ) { _this.items.push('au bureau'); _this. items.push('en transport') }
     if(_this.centre!=null && _this.$page.auth.role=="admin-wilaya" )
     _this.items.push(this.centre['nom'])
    },
    computed:{
    filteredColis(){
     var result=[];
     this.search=this.search.toLowerCase();
      ...
      return this.colis.filter(colis =>{
        ...
        if(colis.livreur!=null)  if(
              colis.livreur['nom'].toLowerCase().match(this.search)!=null || colis.livreur['prenom'].toLowerCase().match(this.search)!=null ) return true; 
    ...
    methods: {
      ...
      refreshIndex(){
          this.$inertia.visit(route('admin.index'))
      ...
      changerEtat(){
        ...
        if((this.$page.auth.role=="admin-wilaya" || this.$page.auth.role=="agent-bureau") ){
         ...
         r = this.selected.filter(element => {
          
          return (element.etat2 != _this.items[2]  )   
          
        });

        if(r.length>0){
          return this.$toast.open({
            message: "Vous avez selectionné des colis qui ne sont pas destinés au centre",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
         ...
         if(this.etat==this.centre['nom']){
          ...
          r = this.selected.filter(element => {
            return !(element.wilaya['id'] !=_this.$page.auth.wilaya['id'] &&  ( element.etat2 == "au bureau" && element.etat1 == "bureau "+this.$page.auth.wilaya['nom']  ))   
          ...
          r = this.selected.filter(element => {
            ...
            return !( element.etat1=="bureau "+_this.$page.auth.wilaya['nom']
               || element.etat1=="pret" || element.etat1=="ramassé" || (element.etat1=="en transport" && element.etat2=="bureau "+_this.$page.auth.wilaya['nom'])
            )  
          });
          if(r.length>0)
          return this.$toast.open({
              message: "Des colis qui ne sont pas au bureau ont été selectionné",
              type: "error",
              duration: 9000,
              dismissible: true,
              position:"top-right"
        ...
        if(this.$page.auth.role=="admin-wilaya" || 
        this.$page.auth.role=="agent-bureau"
        ){
        /*
         ...
         r = this.selected.filter(element => {
          ...
          return (element.etat1 != c  )   
          
        });
        
        if(r.length>0 && this.etat !='au bureau')
        return this.$toast.open({
            message: "Vous avez selectionné des colis qui ne sont pas au bureau",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        */
        ...
        if(this.$page.auth.role=="agent-centre") {
         ...
         if(this.etat!="au centre" && this.etat!="en transport"){
         r = this.selected.filter(element => {
          
          return !(element.etat2 =='au centre' || (element.etat1=="en transport" && element.etat2 != _this.$page.auth.centre['nom']) )
          
        });
        
        if(r.length>0)
        return this.$toast.open({
            message: "Vous avez sélectionné des colis qui n'ont pas été récupérés par le transporteur",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        r=[];
        if(this.etat=="au centre"){
          r = this.selected.filter(element => {
          
          return !(element.etat2 == this.$page.auth.centre['nom'])
          
        });
        if(r.length>0)
        return this.$toast.open({
            message: "Vous avez selectionner des colis qui ne sont destinés au centre",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        if(this.etat=="en transport"){
         r = this.selected.filter(element => {
          ...
          return !(element.etat2 !='au centre' && element.etat1==this.$page.auth.centre['nom'] )
          
        });
        if(r.length>0)
        return this.$toast.open({
            message: "Veuillez changer l'etat du colis avant de l'envoyer",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        r = this.selected.filter(Element => {
          ...
          var x = _this.livreurs.filter(element => {
              if(Element.livreur==null) return false;
              return element.id_livreur == Element.livreur['id_livreur']
          });
          return x.length==0;
        });         
       ...
       if(r.length>0)
        return this.$toast.open({
            message: "Veuillez Assigner a un transporteur",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        if(this.etat=="au centre"){
          ...
          form.etat2=this.etat;
          form.etat=this.$page.auth.centre['nom'];
        }else{
          if(this.etat=="en transport"){
      ...
      attribuerLivreur(){
       ...
       if(this.$page.auth.role=="admin-wilaya" 
         ...
         r = this.selected.filter(element => {
          ...
          return !((element.etat2 == 'au bureau' && element.etat1 == c ) ||element.etat1 == "pret" ||  element.etat1 == "ramassé" || element.etat1 == "en cours de livraison")  
        ...
        if(this.$page.auth.role=="agent-centre"){
          ...
          return element.etat1=="en transport"  
          
        });
        if(r.length>0)
        return this.$toast.open({
            message: "Colis en transport",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        this.$inertia.post('/administration/mescolis/livreur',form).then(()=> {
          this.refreshIndex();
          this.selected=[]
          this.$toast.open({
            message: "Colis modifié avec succés ",
      ...
      filterElements() {
        ...
        alert(this.filtered);
        if(this.filtered)
        this.filteredColis= this.colis.filter(function(element){ 
          return element.wilaya['id'] == _this.$page.auth.wilaya;
        
        });
           
        else this.filteredColis = this.colis;
        alert(this.filteredColis);
        return this.filteredColis;
      },
      filterCentre() {
