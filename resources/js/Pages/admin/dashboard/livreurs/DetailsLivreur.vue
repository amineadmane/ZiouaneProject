    ...
    <v-spacer />
      ...
      <v-main>
        ...
        >
          ...
          <h2 class="mb-2">Les Colis Livrés </h2>
          <v-data-table
            :headers="headers"
            v-model="selectedLiv"
            :single-select="!singleSelect"
            :items="filteredLivres"
            class="elevation-1"
            :page.sync="page"
            :items-per-page="4"
            show-select
          ...
          <template v-slot:[`item.total`]="{ item }">
            {{item.livraison==0?parseInt(item.fraisLivraison)+parseInt(item.prix):item.prix}}
          </template>
          ...
          <h2 class="mb-2">Les Retours </h2> 
            <v-data-table
              :headers="headers"
              v-model="selected"
              :single-select="!singleSelect"
              :items="filteredAnnules"
              class="elevation-1"
              :page.sync="page"
              :items-per-page="4"
              show-select
        ...
        <v-data-table
          ...
          <template v-slot:[`item.details`]="{ item }">
              <v-btn color="primary" 
                text 
                @click="i=indexedItems.indexOf(item);OpenDetails(i)">
                Details
              </v-btn>
            </template>
...
export default {
    props: ['livres','annules','ramassages'],
    components: {
        layout,
        Drawer
    ...
    mounted(){
      var _this = this;
     this.tableItems=Object.values(this.ramassages);
    ...
    filteredAnnules(){
     ...
     this.search=this.search.toLowerCase();
      return this.annules.filter(colis =>{
    ...
    filteredLivres(){
     var result=[];
     this.search=this.search.toLowerCase();
      return this.livres.filter(colis =>{
        if(colis.wilaya!=null)  
            if(colis.wilaya['nom'].toLowerCase().match(this.search) !=null ) 
                {  return true; }
    ...
    filteredColis(){
     ...
     this.search=this.search.toLowerCase();
      return this.indexedItems.filter(colis =>{
        var c = colis[0];
        if(c.client.wilaya!=null && typeof(c.client.wilaya)=='object')  
            if(c.client.wilaya.toLowerCase().match(this.search) !=null ) 
                {  return true; }

        
        
        if(c.client.commune!=null && typeof(c.client.commune)=='object' )  if(
              c.client.commune['nom'].toLowerCase().match(this.search)!=null ) return true; 
        
        if(colis.etat1!=null)  if(
              colis.etat1.toLowerCase().match(this.search)!=null ) return true; 
        
        
         return c.client.nom.toLowerCase().match(this.search)!=null ||
              c.client.prenom.toLowerCase().match(this.search)!=null ||
              c.client.telephone.toLowerCase().match(this.search)!=null ||
              c.client.adresse.toLowerCase().match(this.search)!=null 
    ...
    methods: {
      goBack(){
        this.$inertia.visit('/administration/livreurs');
      },
      OpenDetails(id){
      ...
      DetailsItemAnnules (item) {
        this.editedIndex = this.annules.indexOf(item)
        this.editedItem = Object.assign({}, item)
        this.dialog = true
      },
      DetailsItemLivres(item) {
      ...
      changerEtat(){
        if(this.selected.length==0 && this.selectedLiv.length==0) 
        ...
        this.selected.forEach(element => {
          element.etat2=this.etat;
          this.annules[this.annules.indexOf(element)]=element
          
        });
        else this.selectedLiv.forEach(element => {
          element.etat2=this.etat;
          this.livres[this.livres.indexOf(element)]=element
          
        });
        if(this.selected.length!=0)
        form.selected=this.selected
        else  form.selected=this.selectedLiv

        form.etat=this.etat
        if(this.selectedLiv.length!=0)
