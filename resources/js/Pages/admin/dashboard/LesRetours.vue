      ...
      <v-main>
        ...
        <div class="ml-4">
          <v-row class="">
            ...
            <v-col cols="12" md="3" v-if="$page.auth.role=='agent-centre'">
                <h4>Assigner a un Transporteur</h4>
                <v-select 
                  class="mt-2"
                  :items="livreurs"
                  :item-text="text"
            ...
            <v-col cols="12" md="3">
                <h4 v-if="$page.auth.role!='agent-centre'">Changer l'etat a </h4>
                <h4 v-if="$page.auth.role=='agent-centre'">Envoyer au </h4>
                <v-select 
                  class="mt-2"
                  :items="items"
                  outlined
        ...
        <div class="text-right ml-4">
          ...
          <template v-slot:[`item.nomclient`]="{ item }">
            <v-chip
            color="primary"
            >
              {{item.client['wilaya']['nom']}}
            </v-chip>
          ...
          <template v-slot:[`item.etat1`]="{ item }">
              <v-chip
                color="primary"
              >
                {{item.etat2!=null?  item.motifRetour+' '+item.etat1+" ( "+item.etat2+" )": item.etat1 }}
              </v-chip>
          </template>
...
export default {
    ...
    mounted(){
     ...
     /*
     if(this.centre!=null && this.$page.auth.role=="admin-wilaya" )
     this.items.push(this.centre['nom']);
     */
    
     if(this.$page.auth.role=="agent-bureau" || this.$page.auth.role=="admin-wilaya" ) {  this.items.push('en transport');      }
     if(this.$page.auth.role=="agent-centre") { 
           this.items=this.centres;   
           this.items.push('au centre')
           this.items.push('en transport')
        this.wilayas.forEach(element => {
            this.items.push('bureau '+element);
        }); 
    ...
    computed:{
     ...
     this.search=this.search.toLowerCase();
      result= this.colis.filter(colis =>{
    ...
    methods: {
      ...
      changerEtat(){
        ...
        this.$page.auth.role=="agent-bureau") && this.etat=="en transport"
        ){

         r = this.selected.filter(element => {
          
          return (element.etat2 != _this.centre['nom']  )   
        ...
        if(this.etat!="au centre" ){
         r = this.selected.filter(element => {
           ...
           return ! ( element.etat1 ==this.$page.auth.centre['nom'] ) ;  
          
        });
        alert(r);
        if(r.length>0)
        return this.$toast.open({
            message: "Vous avez sélectionné des colis qui ne sont pas au centre",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        if(this.etat=="en transport") {
          ...
          r = this.selected.filter(Element => {
            x = _this.livreurs.filter(element => {
        ...
        if(r.length>0)
        return this.$toast.open({
            message: "Veuillez attribuer a un transporteur",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        this.$inertia.post(route('admin.changerEtataretourner'),form).then(()=> {
          this.selected=[]
          this.$toast.open({
      ...
      attribuerLivreur(){
        ...
        if(this.$page.auth.role=="agent-centre" 
         ...
         r = this.selected.filter(element => {
          ...
          return ! ( element.etat1 ==this.$page.auth.centre['nom'] || element.etat2 ==this.$page.auth.centre['nom'] ) ;  
          
        });
        
        if(r.length>0)
        return this.$toast.open({
            message: "Vous avez selectionné des colis qui ne sont pas au centre",
            type: "error",
            duration: 9000,
            dismissible: true,
            position:"top-right"
        ...
        this.$inertia.post('/administration/mescolis/livreur/retour',form).then(()=> {
          this.selected=[]
          this.$toast.open({
            message: "Colis modifié avec succés ",
            type: "success",
      ...
      refreshRetour(){
        if(this.$page.auth.role=="agent-centre")
         this.$inertia.visit(route('admin.aretournercentre'))
        else 
        this.$inertia.visit(route('admin.aretourner'))
      },
      filterCentre() {
